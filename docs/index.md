<p align="center">
  <img src="https://raw.githubusercontent.com/Harrys-HQ/ShinKu/main/.github/readme-images/app-icon.png" width="160" alt="ShinKu Logo">
</p>

<h1 align="center">ShinKu</h1>

<p align="center">
  <strong>ShinKu</strong> (Crimson) is a modern, rebranded, and enhanced fork of the SY base, designed for a more immersive and intelligent manga-reading experience.
</p>

---

## 🚀 Latest Release: 2.0.0

The latest version of ShinKu is now available! 

- **[Download APK](https://github.com/Harrys-HQ/ShinKu/releases/latest)**
- **[View Changelog](./CHANGELOG.md)**

---

## ✨ Key Features

*   **Vibe Search (AI Discovery):** Natural language manga search powered by Google Gemini AI.
*   **Reading Journey:** Track your progress with a beautiful statistics card in the "More" tab.
*   **Dropbox Integration:** Secure, cross-device synchronization replacing old methods.
*   **Dynamic Theme Overhaul:** Immersive UI that matches the colors of the manga you are reading.
*   **Smart Categorizer:** Automated library organization into clean, progress-based categories.
*   **Dead Source Scanner:** Quickly find and migrate manga from uninstalled extensions.
*   **Failed Updates Migration:** Dedicated menu to handle manga that failed to refresh.
*   **Library Update Speed Toggle:** Control update concurrency with Standard, Boost, and Extreme modes.
*   **Metadata Refresh Cooldown:** Enforced 7-day cooldown on automatic metadata refreshes to save data.

---

## 🚀 Retained Core Features

ShinKu preserves the best features from its predecessors, TachiyomiSY and Mihon:

*   **Feed Menu:** Pin your favorite searches, popular, and latest updates to a single view.
*   **Manga Recommendations:** Discover new series based on what you are currently reading.
*   **Advanced Filtering:** Lewd filters, tracking filters, and custom source categories.
*   **Metadata Editing:** Edit manga info, tags, and covers directly within the app.
*   **Manga Merging:** Combine multiple versions of the same manga from different sources.

---

## 🛠 Setup & Installation

1.  **Download:** Download the latest APK from the [Releases](https://github.com/Harrys-HQ/ShinKu/releases/latest) page.
2.  **Dropbox Sync:** Configure your Dropbox account in **Settings > Data > Sync** for cross-device reading.
3.  **Vibe Search:** Get an API key from [Google AI Studio](https://aistudio.google.com/app/apikey) and add it in **Settings > Advanced > Vibe Search**.
4.  **Migration:** Coming from SY? Backups are fully compatible!

---

## 📄 Documentation

- **[Full Changelog](./CHANGELOG.md)**
- **[GitHub Repository](https://github.com/Harrys-HQ/ShinKu)**

---

## 📜 Credits & Attributions

ShinKu is built upon the incredible work of the following projects and developers:

*   **[Mihon](https://github.com/mihonapp/mihon) / [Tachiyomi](https://github.com/tachiyomiorg/tachiyomi):** The core foundation of modern open-source manga reading on Android.
*   **[TachiyomiSY](https://github.com/jobobby04/TachiyomiSY):** The primary base for ShinKu, providing advanced library management and E-Hentai/ExHentai support.
*   **[TachiyomiJ2K](https://github.com/arun0009/TachiyomiJ2K):** Inspiration for the dynamic UI elements and immersive layouts.
*   **[Neko](https://github.com/nekomanga/Neko):** Inspiration for the semantic/recommendation discovery concepts.
*   **[Google Gemini](https://aistudio.google.com/):** Powering the AI Vibe Search features.

---

*ShinKu is a community-driven project and is not affiliated with Google or any manga source.*
