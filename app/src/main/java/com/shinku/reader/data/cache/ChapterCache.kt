package com.shinku.reader.data.cache
...
class ChapterCache(
    ...
    private var diskCache = run {
        val perfMode = Injekt.get<BasePreferences>().performanceMode().get()
        val size = if (perfMode == 2) 25L else readerPreferences.cacheSize().get().toLong()
        setupDiskCache(size)
    }
    ...
    init {
        ...
        readerPreferences.cacheSize().changes()
            .drop(1)
            .onEach {
                if (basePreferences.performanceMode().get() == 2) return@onEach
