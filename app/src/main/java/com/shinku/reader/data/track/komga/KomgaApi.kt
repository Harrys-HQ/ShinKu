package com.shinku.reader.data.track.komga
...
class KomgaApi(
    ...
    private val headers: Headers by lazy {
        Headers.Builder()
            .add("User-Agent", "ShinKu v${BuildConfig.VERSION_NAME} (${BuildConfig.APPLICATION_ID})")
            .build()
    }
    ...
    suspend fun getTrackSearch(url: String): TrackSearch =
        withIOContext {
            try {
                ...
                val progress = client
                    .newCall(
                        GET("${url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/com.shinku.reader", headers),
                    )
                    .awaitSuccess().let {
                        with(json) {
    ...
    suspend fun updateProgress(track: Track): Track {
        ...
        client.newCall(
            Request.Builder()
                .url("${track.tracking_url.replace("/api/v1/series/", "/api/v2/series/")}/read-progress/com.shinku.reader")
                .headers(headers)
